...
const EmployerDashboardSection = ({ refreshKey, breaksDefaultOpen = false }: Props) => {
  ...
  const workers = useMemo(() => {
    ...
    for (const r of reports) {
      if (!m.has(r.client_id)) m.set(r.client_id, { id: r.client_id, name: workerNames.get(r.client_id) ?? "Contractor" });
    }
  ...
  const workerBreaks = useMemo(() => {
    ...
    for (const r of reports) {
      ...
      if (!w) {
        w = { id: r.client_id, name: workerNames.get(r.client_id) ?? "Contractor", perDay: new Map() };
        ...
        {breaksOpen && (
          <div className="border-t border-border px-3 py-2 space-y-1.5">
            {workerBreaks.length === 0 ? (
              <p className="text-[11px] text-muted-foreground text-center py-2">No contractor data in this range.</p>
            ) : (
              workerBreaks.map((w) => {
                const color = getClientColor(w.id);
