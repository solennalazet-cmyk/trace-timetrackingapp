...
const SubmittedReportSheet = ({ open, onOpenChange, report, onReviewed, readOnly }: Props) => {
  ...
  const handleReject = async () => {
    ...
    setWorking(false);
    if (error) { toast.error(error.message); return; }
    toast.success("Report rejected. The worker has been notified.");
    setRejectOpen(false);
    setNote("");
        ...
        <AlertDialogContent className="rounded-2xl w-[calc(100vw-2rem)] max-w-[400px]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              The worker will be notified with your reason and can edit and resubmit.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="space-y-3">
            ...
            <Textarea
              placeholder={reason === "other" ? "Required — describe the issue" : "Optional note for the worker"}
              value={note}
